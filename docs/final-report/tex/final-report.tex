...
For example, when a new \tp~is created, this service calls the \textit{fl-tp-types} service through a HTTP client wrapper 
...
With micro services, inner service communication is a given requirement. And since it is communicating outside the system the communication needs to be secured and authenticated. In terms of security, that would be done a https secrit
...
\vspace{\vspaceconst}
\NOTE~talk about creating the db and flApi on startup then they are injected and passed to the controller. this could also be done with the controllers that are needing it only.\\
\NOTE~this is also good because it allows for dependency substitution on testing.\\
\NOTE~if one of the dependencies fails overtime, service crashes and is restarted.\\
