...
\vspace{\vspaceconst}
\NOTE~used flutter because it compiles to native on all platforms, web, linux, windows, macos, android and ios

\subsubsection{API Client}
\vspace{\vspaceconst}
\NOTE~talk about chopper\\
\NOTE~talk about possible service based design which could be implemented and how ive implemented different services based on each endpoint. Specifically how I have different flutter services (classes) for each micro service.\\
\NOTE~this would also be nice so I could only inject each service as needed per page.\\
